...
from utils_celeba_gender import savefig, mkdir
from CelebA import MyCelebA
from classifier_utils_celeba import train, test, adjust_learning_rate, Logger
...
def parse_arguments():
    ...
    parser.add_argument('--model_architecture', '-arch', type=str, default='cnn', help='model architecture')
...
def main():
    ...
    ### load testdata
    num_classes = 2
    transform_test_celeba = transforms.Compose([transforms.Resize((32,32)),
                                                transforms.ToTensor(),
    ...
    if dataset == 'celeba':
        ...
        testset = dataloader(root=DATA_ROOT, split='valid', download=False,
                             transform=transform_test_celeba)
    else:
        raise ValueError
