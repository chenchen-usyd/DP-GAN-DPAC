...
def main():
    ...
    if dataset == 'mnist':
        trainset = datasets.MNIST(DATA_ROOT, download=True, transform=transform, train=False)
    elif dataset == 'fashionmnist':
        trainset = datasets.FashionMNIST(DATA_ROOT, download=True, transform=transform, train=False)
    else:
        raise ValueError
    trainloader = data.DataLoader(trainset, batch_size=args.test_batchsize, shuffle=False, num_workers=args.num_workers)
    ...
    testloader = data.DataLoader(testset, batch_size=args.train_batchsize, shuffle=True, num_workers=args.num_workers)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        torch.save(save_dict, os.path.join(save_dir, 'checkpoint_real2gen.pkl'))
        torch.save(model, os.path.join(save_dir, 'model_real2gen.pt'))

        logger.plot(['Train Loss', 'Val Loss'])
        savefig(os.path.join(save_dir, 'loss_real2gen.png'))

        logger.plot(['Train Acc', 'Val Acc'])
        savefig(os.path.join(save_dir, 'acc_real2gen.png'))
