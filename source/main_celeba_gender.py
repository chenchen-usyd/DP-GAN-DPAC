...
def modify_gradnorm_conv_hook(module, grad_input, grad_output):
    ...
    grad_norm = torch.mean(grad_input_norm) * batchsize
...
def main(args):
    ...
    wc1 = args.wc1
    C1_in = args.C1_in
    C1_fake_iters = args.C1_fake_iters
    C1_real_iters = args.C1_real_iters
    ...
    for iter in range(args.iterations + 1):
        ...
        ### Update C1 network
        #########################
        if wc1 != 0 and iter >= C1_in:

            ##################################### initialise C1 #####################################
            netC1 = ClassifierDCGAN()
            ...
            netC1 = netC1.to(device_C1)
            optimizerC1 = optim.Adam(netC1.parameters(), lr=1e-4, betas=(0.5, 0.9))

            netC1.conv1.register_backward_hook(master_hook_adder)

            for p in netC1.parameters():
                p.requires_grad = True

            ##################################### train C1 using fake data #####################################
            batchsize = 1000
            label = torch.arange(NUM_CLASSES).repeat(batchsize // NUM_CLASSES).to(device0)
            if latent_type == 'normal':
                noise = torch.randn(batchsize, z_dim).to(device0)
            elif latent_type == 'bernoulli':
                noise = bernoulli.sample((batchsize, z_dim)).view(batchsize, z_dim).to(device0)
                ...
                raise NotImplementedError
            noisev = autograd.Variable(noise)
            fake = autograd.Variable(netG(noisev, label.to(device0)).data)

            for iter_c1_fake in range(C1_fake_iters):
                inputv = fake.to(device_C1)
                C_fake_score = netC1(inputv, label.to(device_C1))
                C_fake = -C_fake_score.mean()

                ### add gradient penalty
                gradient_penalty_fake = netC1.calc_gradient_penalty(fake.data, fake.data, label, L_gp, device_C1)
                C1_cost_fake = C_fake + gradient_penalty_fake

                ### add epsilon penalty
                logit_cost_C_fake = L_epsilon * torch.pow(C_fake_score, 2).mean()
                C1_cost_fake += logit_cost_C_fake

                ### update
                C1_cost_fake.backward()
                Wasserstein_C1_fake = -C_fake
                optimizerC1.step()

            ##################################### update C1 using real data #####################################
            input_data = input_pipelines[netD_id] # use the same id as for netD
            for iter_c1_real in range(C1_real_iters):
                real_data, real_y = next(input_data)
                real_data = real_data.view(-1, IMG_DIM)
                real_data = real_data.to(device_C1)
                real_y = real_y.to(device_C1)
                real_data_v = autograd.Variable(real_data)

                ### train with real
                dynamic_hook_function = dummy_hook
                netC1.zero_grad()
                C_real_score = netC1(real_data_v, real_y)
                C_real = -C_real_score.mean()

                ### train with gradient penalty
                gradient_penalty_real = netC1.calc_gradient_penalty(real_data_v.data, real_data_v.data, real_y, L_gp, device_C1)
                C1_cost_real = C_real + gradient_penalty_real

                ### train with epsilon penalty
                logit_cost_C_real = L_epsilon * torch.pow(C_real_score, 2).mean()
                C1_cost_real += logit_cost_C_real

                ### update
                C1_cost_real.backward()
                Wasserstein_C1_real = -C_real
                optimizerC1.step()

            del real_data, real_y, fake, noise, C_real, logit_cost_C_real, gradient_penalty_real, gradient_penalty_fake, inputv, C_fake, logit_cost_C_fake
            torch.cuda.empty_cache()
        ...
        ### get scores from networks
        # C1
        if wc1 != 0 and iter >= C1_in:
            fake = fake.to(device_C1)
            label = label.to(device_C1)
            G_C1 = netC1(fake, label)
        ...
        ### Compute cost
        if (wc1 == 0 or iter < C1_in):
            G = -G_D.mean()
        elif (wc1 != 0 and iter >= C1_in):
            G = -(wd / (wd +wc1)) * G_D.mean() - (wc1 / (wd +wc1)) * G_C1.mean()
        ...
        if wc1 != 0 and iter >= C1_in:
            if iter < 5 or iter % args.print_step == 0:
                print('G_cost:{}, D_cost:{}, C1_cost_fake:{}, C1_cost_real:{}, Wasserstein_D:{}, Wasserstein_C1_fake:{}, Wasserstein_C1_real:{}'.format(
                    G_cost.cpu().data,
                    D_cost.cpu().data,
                    C1_cost_fake.cpu().data,
                    C1_cost_real.cpu().data,
                    Wasserstein_D.cpu().data,
                    Wasserstein_C1_fake.cpu().data,
                    Wasserstein_C1_real.cpu().data
                    ))

        if wc1 == 0 or iter < C1_in:
            if iter < 5 or iter % args.print_step == 0:
                print('G_cost:{}, D_cost:{}, Wasserstein_D:{}'.format(G_cost.cpu().data,
                                                                      D_cost.cpu().data,
                                                                      Wasserstein_D.cpu().data,
                                                                      ))

        if iter % args.vis_step == 0:
            generate_image(iter, netGS, fix_noise, save_dir, device0, num_classes=2)

        if iter % args.save_step == 0:
            ### save model
            torch.save(netGS.state_dict(), os.path.join(save_dir, 'netGS_%d.pth' % iter))
            save_gen_data(os.path.join(save_dir, 'gen_data_%d.npz' % iter), netGS, z_dim, device0,
