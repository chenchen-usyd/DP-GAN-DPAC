...
def parse_arguments():
    ...
    parser.add_argument('--dataset', '-data', type=str, default='mnist', choices=['mnist', 'fashionmnist', 'celeba_gender'], help='dataset name')
    ...
    parser.add_argument('--wd', '-wd', type=float, default=1.0, help='discriminator weight')
    parser.add_argument('--wc1', '-wc1', type=float, default=0.0, help='C1 weight')
    parser.add_argument('--C1_in', '-C1in', type=float, default=15000, help='from which iteration C1 get involved in G training')
    parser.add_argument('--L_epsilon', '-lep', type=float, default=0.001, help='epsilon penalty (used in PGGAN) for discriminator')
    parser.add_argument('--critic_iters', '-diters', type=int, default=5, help='number of critic iters per gen iter')
    parser.add_argument('--C1_fake_iters', '-C1_fake_iters', type=int, default=300, help='number of C1 iters per gen iter')
    parser.add_argument('--C1_real_iters', '-C1_real_iters', type=int, default=5, help='number of C1 iters per real iter')
    parser.add_argument('--latent_type', '-latent', type=str, default='bernoulli', choices=['normal', 'bernoulli'], help='latent distribution')
    ...
    parser.add_argument('--gen_arch', '-gen', type=str, default='ResNet', choices=['DCGAN', 'ResNet'], help='generator architecture')
