...
IMG_W = IMG_H = 32  # image width and height
...
class GeneratorDCGAN(nn.Module):
    def __init__(self, z_dim=10, model_dim=64, num_classes=2, outact=nn.Sigmoid()):
...
class GeneratorResNet(nn.Module):
    ...
    def forward(self, z, y):
        ...
        output = torch.reshape(output, [-1, IMG_C * IMG_H * IMG_W])
        return output


class DiscriminatorDCGAN(nn.Module):
    def __init__(self, model_dim=64, num_classes=2, if_SN=True):
        ...
        if if_SN:
            self.conv1 = SpectralNorm(nn.Conv2d(IMG_C, model_dim, 5, stride=2, padding=2))
            ...
            self.conv1 = nn.Conv2d(IMG_C, model_dim, 5, stride=2, padding=2)
    ...
    def forward(self, input, y):
        input = input.view(-1, IMG_C, IMG_W, IMG_H) # n,3,32,32
        h = self.relu(self.conv1(input)) #n,model_dim,16,16
        h = self.relu(self.conv2(h)) #n,model_dim*2,8,8
        h = self.relu(self.conv3(h)) #n,model_dim*4,4,4
        h = h.view(-1, 4 * 4 * 4 * self.model_dim) #n,model_dim*4*4*4
        out = self.linear(h) #n,1
        out += torch.sum(self.linear_y(y) * h, dim=1, keepdim=True) #n,1
        return out.view(-1)
...
class ClassifierDCGAN(nn.Module):
    def __init__(self, model_dim=64, num_classes=2, if_SN=True):
        super(ClassifierDCGAN, self).__init__()
        ...
        if if_SN:
            ...
            self.conv2 = SpectralNorm(nn.Conv2d(model_dim, model_dim * 2, 5, stride=2, padding=2))
            self.conv3 = SpectralNorm(nn.Conv2d(model_dim * 2, model_dim * 4, 5, stride=2, padding=2))
            self.linear = SpectralNorm(nn.Linear(4 * 4 * 4 * model_dim, num_classes))
        else:
            self.conv1 = nn.Conv2d(IMG_C, model_dim, 5, stride=2, padding=2)
            self.conv2 = nn.Conv2d(model_dim, model_dim * 2, 5, stride=2, padding=2)
            self.conv3 = nn.Conv2d(model_dim * 2, model_dim * 4, 5, stride=2, padding=2)
            self.linear = nn.Linear(4 * 4 * 4 * model_dim, num_classes)
        self.relu = nn.ReLU()

    def forward(self, input, y):
        input = input.view(-1, IMG_C, IMG_W, IMG_H) # n,1,28,28
        h = self.relu(self.conv1(input)) #n,model_dim,14,14
        h = self.relu(self.conv2(h)) #n,model_dim*2,7,7
        h = self.relu(self.conv3(h)) #n,model_dim*4,4,4
        h = h.view(-1, 4 * 4 * 4 * self.model_dim) #n,model_dim*4*4*4
        out = self.linear(h) #n,10

        y_onehot = F.one_hot(y, num_classes=self.num_classes)
        y_onehot = torch.where(y_onehot==1, torch.tensor(1.).to(y_onehot.device), torch.tensor(-1.).to(y_onehot.device))
