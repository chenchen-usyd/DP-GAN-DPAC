...
def generate_image(iter, netG, fix_noise, save_dir, device, num_classes=2, img_c=3,
                   img_w=32, img_h=32):
    ...
    for class_id in range(num_classes):
        ...
        sample = sample.view(batchsize, img_c, img_w, img_h)
        sample = sample.cpu().data.numpy()
        sample_list.append(sample)
    samples = np.transpose(np.array(sample_list), [1, 0, 3, 4, 2])
    samples = np.reshape(samples, [nrows * ncols, img_w, img_h, img_c])
...
def save_gen_data(path, netG, z_dim, device, latent_type='bernoulli', img_w=32, img_h=32, img_c=3, num_classes=2,
